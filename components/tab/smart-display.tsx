...
export function SmartDisplay({children,orderId,status,copy,tabTotal,onTab,simulator,onSimulatorChange}:{children:ReactNode;orderId:number;status:string;copy:string;tabTotal:string;onTab:()=>void;simulator:boolean;onSimulatorChange:(enabled:boolean)=>void}){
 ...
 useEffect(()=>{if(permission!=='granted')return;const timer=setInterval(()=>setTimedOut(performance.now()-lastEvent.current>3000),1000);return()=>clearInterval(timer)},[permission]);
 ...
 useEffect(()=>{
  ...
  const render=(time:number)=>{
   ...
   if(pending.current){const sample=pending.current;pending.current=null;if(!cfg.useSimulator)receiveTilt(s,sample.degrees,sample.time)}
   const progress=stepFlow(s,dt),paint=reduced.matches?(progress<.5?0:1):progress;
 ...
 return <div className="resin-display">
  <div className={`resin-stage ${status==='COMING TO YOU'?'resin-coming':''}`} ref={stage} aria-label="Live physical order marker">
   ...
   <button className="resin-manual" ref={manual} aria-label="Move order to top" title="Flip server side" onClick={moveManually}>
    <svg ref={upIcon} width="30" height="36" viewBox="0 0 30 36" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="M15 15V3m-5 5 5-5 5 5"/><rect x="4" y="21" width="22" height="11" rx="3"/></svg>
    <svg ref={downIcon} style={{display:'none'}} width="30" height="36" viewBox="0 0 30 36" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><rect x="4" y="4" width="22" height="11" rx="3"/><path d="M15 21v12m-5-5 5 5 5-5"/></svg>
   </button>
   {(permission==='prompt'||permission==='denied'||(permission==='granted'&&timedOut&&available))&&<div className="resin-permission"><button disabled={requesting} onClick={enableMotion}>{requesting?'Enabling…':'Enable Motion'}</button><p>{permission==='denied'?'Motion access is off. The arrow always works.':'Lets TAB respond when you tilt your phone.'}</p></div>}
  </div>
  <div className="resin-footer"><div className="resin-status" aria-live="polite"><strong>{status}</strong><p>{copy}</p></div><div className="resin-footer-row"><span ref={liveLabel}>{flow.current.manual?'Manual position':events?'Smart Motion on':permission==='granted'?'Waiting for motion':available?'Manual control':'Motion unavailable'}</span><button onClick={onTab}>TAB · {tabTotal}</button></div></div>
