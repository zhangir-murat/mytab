...
export type Order = { id:number; number:number; venue:string; venueId?:string; items:Line[]; subtotal:number; tax:number; tip:number; total:number; paymentStatus:'paid'|'on-tab'; paymentMethod:string; createdAt:number; status:Status; color:Color; deliveryItems:Line[]; deliveredCount:number; tabId:string };
export type Receipt = { id:string; venue:string; venueId?:string; closedAt:number; orders:Order[]; subtotal:number; tax:number; tip:number; total:number; paidBefore:number; charged:number; paymentMethod:string };
...
export const storageKey=(venueId:string)=>venueId==='elsewhere'?STORAGE:`tab-${venueId}-v2`;
...
export function placeOrder(s:Session,lines:Line[],fromCart:boolean,now=Date.now(),venue:{id:string;name:string}={id:'elsewhere',name:'Elsewhere'}){
 ...
 const order:Order={id:s.nextId,number,venue:venue.name,venueId:venue.id,items,subtotal,tax,tip:0,total:subtotal+tax,paymentStatus:'on-tab',paymentMethod:s.authorizedMethod,createdAt:now,status:'new',color,deliveryItems:items.slice(0,2),deliveredCount:0,tabId:s.tabId};
...
export function settleTab(s:Session,tip:number,now=Date.now(),venue:{id:string;name:string}={id:'elsewhere',name:'Elsewhere'}){
 ...
 const receipt:Receipt={id:s.tabId,venue:venue.name,venueId:venue.id,closedAt:now,orders:closedOrders,subtotal:t.subtotal,tax:t.tax,tip:t.legacyTips+tip,total:t.total+tip,paidBefore:t.paidBefore,charged:t.due+tip,paymentMethod:s.authorizedMethod};
...
export function restoreSession(value:unknown):Session{
 ...
 for(const [id,rows] of groups){if(id===currentId)continue;const t=totals(rows);receipts.push({id,venue:rows[0].venue||'Elsewhere',venueId:rows[0].venueId||'elsewhere',closedAt:Math.max(...rows.map(o=>o.createdAt)),orders:rows,subtotal:t.subtotal,tax:t.tax,tip:t.legacyTips,total:t.total,paidBefore:t.total,charged:0,paymentMethod:rows[0].paymentMethod})}
